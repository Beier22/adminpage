<template>
    ...
    <div>
        ...
        <li v-for="pizza in pizzas" v-bind:key="pizza.id" style="list-style: none">
            <router-link :to="`/editpizza/${pizza.id}`" tag="button">{{pizza.name}}</router-link>
        </li>
        <h2>Fastfoods</h2>
        <li v-for="fastfood in fastfoods" v-bind:key="fastfood.id" style="list-style: none">
            <router-link :to="`/editfastfood/${fastfood.id}`" tag="button">{{fastfood.name}}</router-link>
        </li>
        <h2>Drinks</h2>
        <li v-for="drink in drinks" v-bind:key="drink.id" style="list-style: none">
            <router-link :to="`/editdrink/${drink.id}`" tag="button">{{drink.name}}</router-link>
        </li>
        <h3>{{message}}</h3>
    </div>
</template>
